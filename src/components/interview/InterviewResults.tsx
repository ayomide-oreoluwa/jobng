...
export const InterviewResults: React.FC<InterviewResultsProps> = ({ 
  ...
  // Grade classification (Pass benchmark >= 70%)
      ...
      <div className={`border rounded-3xl p-6 sm:p-8 text-center space-y-6 shadow-md transition-all ${
        ...
        <div>
          <div className="inline-block px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider mb-2 border">
            {isPassed ? (
              <span className="bg-emerald-100 text-emerald-800 border-emerald-200 px-3 py-1 rounded-full flex items-center gap-1.5 inline-flex">
                <FiCheckCircle className="w-3.5 h-3.5" /> Interview Passed
              </span>
            ) : (
              <span className="bg-rose-100 text-rose-800 border-rose-200 px-3 py-1 rounded-full flex items-center gap-1.5 inline-flex">
        ...
        {/* Explanatory Performance Message */}
